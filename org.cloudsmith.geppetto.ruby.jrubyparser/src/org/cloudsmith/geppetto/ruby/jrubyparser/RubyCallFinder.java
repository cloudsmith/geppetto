...
/**
 * Finds calls in a parsed ruby AST. An instance of this class can be reused, but it is not threadsafe.
 * 
 * Calls are found using a FQN - i.e. a sequence of module/receiver names, where the last name segment
 * is the name of the function. The search will find function calls irrespective of call type (e.g.
 * a FCallNode (where receiver is implied), or CallNode where receiver is explicit. All FQN names are 
 * appended to the current scope - i.e. if a call is made to X::Y::foo() in module A, it will be found by 
 * a search of A::X::Y::foo().
 * 
 * TODO: global references are not handled - i.e. if a call to ::X::Y::foo() is made inside module A, it will
 * be recognized (in error) as A::X::Y::foo() - also see {@link ConstEvaluator}.
 */
public class RubyCallFinder {
	...
	private boolean inCompatibleScope() {
		...
		// i.e. we find module a::b::c::d(::_)* when we are looking for a::b::c::FUNC
		//
		if(nameStack.size() >= qualifiedName.size())
			return false; // will not be found in this module - it is out of scope
		...
		try {
			return qualifiedName.subList(sizeX-sizeY, sizeX).equals(nameStack) ? true : false;
	...
	private boolean inWantedScope() {
		// we are in wanted scope if all segments (except the function name) match.
		try {
			return qualifiedName.subList(1, qualifiedName.size()).equals(nameStack) ? true : false;
		} catch(IndexOutOfBoundsException e) {
			return false;
		}
	}
	...
	/**
	 * Pushes a name onto the name stack (and the scope stack, as we need to discard the names
	 * when leaving a named scope).
	 * @param name
	 */
	...
	/**
	 * Pops a name of the namestack - should not be called without also managing the scope stack.
	 */
